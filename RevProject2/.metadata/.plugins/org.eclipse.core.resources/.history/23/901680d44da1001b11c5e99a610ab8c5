...
public class RecipeController {
	...
	public ResponseEntity<Recipes> updateRecipe(@PathVariable(value = "recipe_id") long recipe_id,
											@Valid @RequestBody Recipes recipeDetails) throws ResourceNotFoundException {
		Recipes recipe = recipeRepository.findById(recipe_id)
		.orElseThrow(() -> new ResourceNotFoundException ("User not found for this id :: " + recipe_id));
		
		recipe.setRecipe_name(recipeDetails.getRecipe_name());
		recipe.setSkill_level(recipeDetails.getSkill_level());
